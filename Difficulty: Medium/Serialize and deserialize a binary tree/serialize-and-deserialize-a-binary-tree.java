...
class Tree {
    ...
    private void serializeHelper(Node root, ArrayList<Integer> ans) {
        if (root == null) {
            ans.add(-1); // Using -1 as a marker for null
            return;
        }
        ans.add(root.data);
    ...
    private Node deSerializeHelper(ArrayList<Integer> arr, Index index) {
        if (index.value >= arr.size() || arr.get(index.value) == -1) {
            index.value++;
            return null;
        }
        Node root = new Node(arr.get(index.value++));
